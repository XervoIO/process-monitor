...
module.exports.monitor = function(config) {
  if (typeof config.pid === 'undefined' && typeof config.pids === 'undefined') {
    throw new TypeError('You must specify at least one PID');
  } else if (config.pid && typeof config.pids === 'undefined') {
    config.pids = [];
    config.pids[0] = config.pid;
  }
